...
#=
    ...
    BCS-form loss (issues #8 / #3). The 1x2 unit cell must reproduce the 1x1 result
...
=#
@testset "Kitaev HC model on larger unit cells" begin
    Jx = Jy = Jz = 1.0

    @testset "Exact energy folding (1x2 vs 1x1)" begin
        lattice_1x1 = InfiniteRectLattice(1, 1; N_kx=100, N_ky=100, bc=(:PBC, :APBC))
        lattice_1x2 = InfiniteRectLattice(1, 2; N_kx=100, N_ky=50, bc=(:PBC, :APBC),
            uc_layout=reshape([1, 2], 2, 1))
        ...
        @test E_1x1 ≈ -1.5746 atol=1e-4 # fluxfree sector reference value per (honeycomb) unit cell
        @test E_1x2 / GfPEPS.get_number_of_sites(lattice_1x2) ≈ E_1x1 atol=1e-10
    end;

    @testset "CM optimization (1x2 vs 1x1)" begin
        Nf, Λ = 1, 2
        lattice_1x1 = InfiniteRectLattice(1, 1; N_kx=12, N_ky=12, bc=(:PBC, :APBC))
        lattice_1x2 = InfiniteRectLattice(1, 2; N_kx=12, N_ky=6, bc=(:PBC, :APBC),
            uc_layout=reshape([1, 2], 2, 1))
